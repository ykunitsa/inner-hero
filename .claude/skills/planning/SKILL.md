...
2. **Read the current state.** Relevant kept code (`Inner Hero/`), `TECH_DEBT.md`.
   Never look into `_to_delete/` for reference — that design is dead.
3. **Write the plan** with these sections, kept short:
...
4. **Principles check.** End with one line per violated-or-risky principle, or
...
5. Present the plan and wait for confirmation before implementing
