...
private struct ActivationCategoryDTO: Codable {
    ...
    init(_ c: ActivationCategory) {
        id = c.id; predefinedKey = c.predefinedKey; title = c.title; sfSymbol = c.sfSymbol
        colorHex = c.colorHex; sortOrder = c.sortOrder; isPreset = c.isPreset; createdAt = c.createdAt
    }
}
private struct ActivationTaskDTO: Codable {
    ...
    init(_ t: ActivationTask) {
        id = t.id; categoryId = t.categoryId; predefinedKey = t.predefinedKey; title = t.title
        hint = t.hint; pleasureTag = t.pleasureTag; masteryTag = t.masteryTag
        effortLevel = t.effortLevelRaw; suggestedMinutes = t.suggestedMinutes; sfSymbol = t.sfSymbol
        isPreset = t.isPreset; isHiddenByUser = t.isHiddenByUser; sortOrder = t.sortOrder
...
private struct ActivationSessionDTO: Codable {
    ...
    init(_ s: ActivationSession) {
        id = s.id; activityId = s.activityId; assignmentId = s.assignmentId; statusRaw = s.statusRaw
        moodBefore = s.moodBefore; moodAfter = s.moodAfter; moodDelta = s.moodDelta
        barrierNote = s.barrierNote; reflectionNote = s.reflectionNote
        plannedFor = s.plannedFor; startedAt = s.startedAt; completedAt = s.completedAt
        actualMinutes = s.actualMinutes; createdAt = s.createdAt
    }
}
...
#Preview {
    ...
    .modelContainer(for: [
        ...
        ActivationTask.self, ActivationSession.self,
        ExerciseAssignment.self, FavoriteExercise.self
    ], inMemory: true)
}
